...
function main() {
  ...
  const expectedMp4Paths = entries.map((entry) => entry.path).sort();
  ...
  if (rootVersion === "1.0.0" && entries.length !== 7) failures.push("v1.0.0 media pack must contain exactly 7 released MP4 files");
  if (failures.length) {
    console.error(`Media-pack validation failed with ${failures.length} issue(s):`);
    for (const failure of failures) console.error(`- ${failure}`);
    process.exit(1);
  }
  console.log(`Media-pack validation passed (${entries.length} hash-bound, fully decoded MP4 files; audio required=${requireAudio}).`);
}
