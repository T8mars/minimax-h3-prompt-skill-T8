...
function sniffMedia(buffer, extension) {
  ...
  if (buffer.length >= 4 && buffer[0] === 0x1a && buffer[1] === 0x45 && buffer[2] === 0xdf && buffer[3] === 0xa3) return { kind: "video", mimeType: "video/webm", extension: "webm" };
  return null;
}
...
class PromptMediaStore {
  ...
  addPaths(paths) {
    ...
    for (const value of candidates) {
      ...
      if (!type) throw new Error("Unsupported or invalid reference media. Use PNG, JPEG, WebP, MP4, MOV, or WebM.");
