...
function automaticUpdateDelay({ isPackaged, platform = process.platform, env = process.env }) {
  if (!isPackaged || platform === "darwin" || env.T8_DISABLE_AUTO_UPDATE === "1") return null;
