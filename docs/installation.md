...
```powershell
Get-FileHash .\T8-Prompt-Library-Setup-v1.3.7.exe -Algorithm SHA256
```
...
```bash
shasum -a 256 T8-Prompt-Library-v1.3.7-mac-universal.dmg
```
...
Expand-Archive .\prompt-library-media-v1.3.7-part1.zip -DestinationPath $mediaRoot -Force
Expand-Archive .\prompt-library-media-v1.3.7-part2.zip -DestinationPath $mediaRoot -Force
```
