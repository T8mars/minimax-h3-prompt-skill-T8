...
v1.2.2 在本地 `localStorage` 保存显示语言、用户明确选择的目录排序，以及用户主动创建的收藏、合集与最近 100 条浏览历史。API Workbench 的默认渠道与非秘密设置保存在本机；用户选择长期保存的 Key 仅通过 Electron `safeStorage` 加密保存，不进入渲染进程、命令行、日志、项目导出或案例目录。个人资料库只记录公开条目的稳定 ID、合集名称和时间戳；不复制提示词正文、来源页面内容、账号或媒体文件，不上传、不同步，也不进入 Git 仓库。搜索词和筛选条件仍不持久保存。删除合集不会删除公共内容；清空应用站点数据会恢复默认语言、默认排序并移除全部个人资料库数据。
