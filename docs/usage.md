...
- 默认“全部”入口统一显示 117 项：106 个创意案例、9 个 MiniMax 官方仓库条目与 2 个非官方用户贡献 Skills，并保留三个独立分类入口；
...
工作台把本地机制推荐、模板选择、目标模型、参考素材、三渠道提示词增强、静态锚点检查与实验项目保存放在一起。它只生成提示词，不生成视频。

关键边界：

- 默认中文，随应用语言切换完整中英文 UI；
...
- 预检确认单会分别列出一次 Chat 和平价小屋需要的素材上传数；
- 确认单只能使用一次，重复点击不会产生第二次请求；
- 运行中取消、读取超时或响应丢失时，不宣称远端已停止或不会计费；
- 输出保留完整文本，并把语法、用户事实和机制锚点的可确定检查与人工判断分开；
