...
- 默认“全部”入口统一显示 227 项：216 个创意案例、9 个 MiniMax 官方仓库条目与 2 个非官方用户贡献 Skills，并保留三个独立分类入口；
...
工作台在同一窗口提供相互隔离的视频提示词和 MiniMax Music 3 两项能力。它只生成文本，不生成视频或音频。视频使用当前云端 API 或本地 Qwen 对统一总索引进行两阶段语义推荐，并提供模板、双模型与参考素材；Music 3 提供歌词模式、结构参数、官方 Skill 渐进读取和四项结果。
...
- 推荐、AI 分镜、双机制组合、双向 Music 桥接和模板改进提案都明确显示模型调用次数；无可用渠道时不会退回本地猜测；
- 统一总索引逐条记录所有案例、官方 Skill、社区 Skill 和证据变体；任何新增入库遗漏都会使目录校验失败；
