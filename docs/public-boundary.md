...
`validate-catalog.mjs` 进一步要求公开案例状态为 `released`；`validate-official-skills.mjs` 固定检查 9 个上游入口、哈希、来源分类、Seedance 伴侣和 `comfyui_import=false`；`validate-skills.mjs` 检查 Skill 名称、frontmatter、摘要、模板及 Agent 元数据。
